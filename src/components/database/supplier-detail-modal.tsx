...
function LockedField({
  ...
  return (
    <Link
      href="/pricing"
      onClick={(e) => e.stopPropagation()}
      className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-slate-200 bg-slate-50/50 hover:bg-teal-50/50 hover:border-teal-200 transition-all group shadow-sm"
      title={`Upgrade to ${plan} to unlock ${label}`}
    >
      <Lock className="h-3.5 w-3.5 text-slate-400 group-hover:text-teal-600 transition-colors" />
      {!compact && (
        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 group-hover:text-teal-600 transition-colors">
          {plan}
        </span>
      )}
    </Link>
...
function ModalLockedBadge({
  ...
  return (
    <Link
      href="/pricing"
      className={`inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 text-slate-500 hover:border-slate-300 hover:bg-slate-100 transition-colors ${compact ? "px-2.5 py-1 text-xs" : "px-3 py-2 text-sm"}`}
    >
