...
export function PublicBlogList({ posts }: { posts: Post[] }) {
  ...
  const filtered = useMemo(() => posts.filter((post) => (category === "All" || post.category === category) && (!query || `${post.title} ${post.excerpt ?? ""}`.toLowerCase().includes(query.toLowerCase()))), [posts, query, category]);
  return (
            ...
            <Link href={`/blog/${post.slug}`} className="block">
              ...
              <div className="p-5">
                <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400">
                  <span>{post.category || "Sourcing Research"}</span>
                  {post.publishedAt && (
                ...
                <h2 className="mt-3 text-lg font-bold text-slate-950 group-hover:text-teal-700">{post.title}</h2>
