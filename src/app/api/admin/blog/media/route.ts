...
export async function POST(request: Request) {
  try {
    ...
    if (!publicBaseUrl) return NextResponse.json({ error: "尚未配置图片存储" }, { status: 503 });
    ...
    const item = await prisma.blogMedia.create({ data: { key, publicUrl: `${publicBaseUrl.replace(/\/$/, "")}/${key}`, fileName: file.name, contentType: file.type, uploadedBy: user.id } });
