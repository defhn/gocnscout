...
async function main() {
  console.log("Starting DeepSeek AI Remaining Cities (Noindex/腰部城市) Description Generator...");
  ...
  // 查出所有 isIndexable: false 的腰部城市记录
  const cities = await prisma.cityPage.findMany({
    where: { isIndexable: false },
    orderBy: { supplierCount: "desc" }
  });

  console.log(`Found ${cities.length} remaining (noindex/腰部) cities to process.`);
...
- Output MUST be a valid JSON object containing exactly two keys:
  ...
  console.log("\nAll remaining noindex cities processed successfully!");
}
